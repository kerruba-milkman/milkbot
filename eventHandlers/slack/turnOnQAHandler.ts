...
export async function handleTurnOnQA(i: BotMessage): Promise<BotMessage> {
    i.expectedResponseType = 'chat'

    let userWorflow: ChatPostMessageArguments = {
        channel: i.requestChannelId,
        text: 'Turn on QA Environement',
        blocks: [
            {
                ...
                text: {
                    ...
                    text: "Hello, do you *really* want to turn on QA Environment?"
                }
            },
            {
...
export async function turnOnQAAction(i: BotMessage): Promise<BotMessage> {
